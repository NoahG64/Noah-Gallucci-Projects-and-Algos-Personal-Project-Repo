...
public class User {
	...
	@Pattern(regexp = "^(?=.*\\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[^\\w\\d\\s:])([^\\s]){8,128}$", message="Password must contain at least 1 Uppercase character, 1 lowercase letter, 1 number and 1 non-alphanumeric character.")
